...
def cloning_machine(coefficients, wire):
    ...
    # prepare the state
    #qml.QubitStateVector([(coefficients[0]+coefficients[1])/np.sqrt(2),coefficients[1]/np.sqrt(2) , 0,coefficients[0]/np.sqrt(2)], wires=[1, 2])
    c0 = coefficients[0]
    c1 = coefficients[1]
    ...
    theta1 = 2*np.arccos(np.sqrt(alpha**2 + beta**2))
    theta2 = 2*np.arccos(alpha/np.sqrt(alpha**2 + beta**2))

    qml.RY(theta1, wires=1)
    qml.RY(theta2, wires=2)
    qml.CNOT([1,2])
    qml.RY(-(np.pi-theta2)/2, wires=2)
    qml.CNOT([1,2])
    qml.RY((np.pi-theta2)/2, wires=2)


    #Apply the cloning machine
    qml.CNOT([0,1])
    qml.CNOT([0,2])
    qml.CNOT([1,0])
    qml.CNOT([2,0])
...
def fidelity(coefficients):
    ...
    return np.array([fidelity_0, fidelity_1])
